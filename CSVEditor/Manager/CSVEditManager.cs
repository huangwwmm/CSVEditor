...
public class CSVEditManager
{
	...
	4         7
	*/
	#region Copy\Cut\Paste\InsertFromClipboard
	public void Copy()
	{
		if (!CanCopy())
		{
	...
	public void Paste()
	{
		...
		{
			PasteCell(dataGridView, clipboardText);
		...
		else if (MainForm.Instance.GetCellEditTextBox().Focused)
		{
			...
			newValue = newValue.Insert(selectionStart, clipboardText);
		...
		else if (dataGridView.SelectedCells.Count > 0)
		{
			...
			{
				PasetCells(dataGridView, clipboardText);
			}
			else
			{
	...
	public bool CanInsertFromClipboard()
	{
		if (!GetclipboardText(out string clipboardText))
		{
			return false;
	...
	private void PasteCell(DataGridView dataGridView, string clipboardText)
	{
		m_CSVForm.BeforeChangeCellValue();
	...
	private void PasetCells(DataGridView dataGridView, string clipboardText)
	{
		// 引用下面两个链接
	...
	private bool GetclipboardText(out string clipboardText)
	{
		clipboardText = null;
		// 获取剪切板的数据
		try
