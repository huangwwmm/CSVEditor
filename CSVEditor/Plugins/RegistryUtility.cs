...
public class RegistryUtility
{
	...
	public static void SetRegisterFileExtendWithThisApp(string fileExtend, string progID, string progDescription, string playDescription)
	{
		...
		try
		{
			...
			{
				classesRootKey.DeleteSubKeyTree(progID);
			}

			progIDKey = classesRootKey.CreateSubKey(progID, true);
			// 默认键值，显示给用户看的文件类型描述
			progIDKey.SetValue("", progDescription, RegistryValueKind.String);

			// 文件显示的图标
			defaultIconKey = progIDKey.CreateSubKey("DefaultIcon", true);
			// 默认键值，指定文件显示图标
			defaultIconKey.SetValue("", Application.ExecutablePath + ",0", RegistryValueKind.String);

			// shell//open//command
			shellKey = progIDKey.CreateSubKey("shell", true);
			openKey = shellKey.CreateSubKey("open", true);
			commandKey = openKey.CreateSubKey("command", true);
			commandKey.SetValue("", "\"" + Application.ExecutablePath + "\" \"%1\"", RegistryValueKind.String);

			// shell//play//command
			playKey = shellKey.CreateSubKey("play", true);
			playKey.SetValue("", playDescription, RegistryValueKind.String);
			commandKey = playKey.CreateSubKey("command", true);
			commandKey.SetValue("", "\"" + Application.ExecutablePath + "\" \"%1\"", RegistryValueKind.String);
		}
		catch (Exception ex)
		{
		...
		try
		{
			...
			{
				fileExtendKey = classesRootKey.CreateSubKey(fileExtend, true);
			}
